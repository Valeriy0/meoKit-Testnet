...
export const Reflink = ({ checkNft, isAllowReflink, wrapperStyle = '', inputStyle = '' , withoutShare = true }) => {
  ...
  const refBlock = useMemo(() => {
    ...
    return (
      <>
          <div className={`whitespace-nowrap max-w-[330px] w-full sm:max-w-full flex items-center justify-between font-medium p-1 pl-3 bg-[#1C1D1E] rounded-[16px] ${inputStyle}`}> 
            <span className="scroll-hidden max-w-[286px] sm:max-w-full w-full overflow-auto"> https://presale.frgx.space?ref={account} </span>
            <button onClick={() => copy(`https://presale.frgx.space?ref=${account}`)} className="ml-2.5 rounded-[12px] flex-shrink-0 w-10 h-10 bg-[#1F80F2] flex items-center justify-center">
